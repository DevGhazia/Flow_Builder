...
export default function MessageNode({id, data}){
    ...
    const {selectedNodeId} = useSelector((state)=>state.flow);
    return (
        <>
            {/* Node Container */}
            <div 
                className={`flex flex-col w-60 drop-shadow-lg border-2 hover:border-blue-400 rounded-[4px] overflow-hidden 
                    ${id===selectedNodeId? 'hover:border-blue-800 border-blue-800' : ''}`}
                tabIndex={0}
            ...
            >
                ...
                {/* Node Text Area */}
                <div className='p-2 flex bg-white min-h-10'>
                    <span className='text-sm'>
                        {data.text}
                    </span>
                </div>
            </div>
            <Handle type='source' position='right' style={{ height:'10px', width:'10px'}}/>
            <Handle type='target' position='left' style={{ height:'10px', width:'10px'}}/>
        </>
    )
}
